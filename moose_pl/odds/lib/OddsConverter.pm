...
sub _format_oddsconverter
{
   my $self = shift;

   $self->_format_roi();
   $self->_format_decimal_odds();
...
sub _invoke_decimal_odds
{
   ...
   ## Validation out of Moose scope
   if ($self->{'decimal_odds'} < $OddsConverter::MIN_DECIMAL_ODDS) {
...
sub _invoke_roi
{
   ...
   ## Validation out of Moose scope
   if ($self->{'roi'} < $OddsConverter::MIN_ROI) {
...
sub _invoke_probability
{
   ...
   ## Validation out of Moose scope
   if ($self->{'probability'} < $OddsConverter::MIN_PROBABILITY) {
